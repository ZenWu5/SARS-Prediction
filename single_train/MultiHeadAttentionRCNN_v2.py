...
def visualize_attention(model, data_loader, n_samples=5):
    ...
    for i, sample in enumerate(samples):
        ...
        # 绘制注意力权重
        print(sample['attention'][:, 1].shape, sample['seq_len'])
        plt.bar(
        x=list(range(sample['seq_len'])), 
        height=sample['attention'][:, 1],
        width=1.0
        )
        plt.xlabel('序列位置')
        plt.ylabel('注意力权重')
...
def main():
    ...
    logger.info(f"HDF5 样本总数: {data_num}, pad 长度={dataset.max_seq_len}, emb_dim={dataset.emb_dim}")
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        ...
        for i, res in enumerate(fold_results):
            f.write(
                f"第 {i+1} 折: "
                f"loss={res.get('loss', float('nan')):.6f}, "
                f"MSE={res.get('mse', float('nan')):.6f}, "
                f"MAE={res.get('mae', float('nan')):.6f}, "
                f"R²={res.get('r2', float('nan')):.6f}, "
                f"Pearson={res.get('pearson', float('nan')):.6f}\n"
            )
