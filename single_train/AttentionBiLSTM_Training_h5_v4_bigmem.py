...
class InMemoryDataset(Dataset):
    def __init__(self, subset, device=None, preload=0):
        """
        高效内存缓存数据集，可选将部分数据预加载到GPU
        
        参数:
            subset: 原始数据集子集
            device: 若指定，则将前preload个样本预加载到GPU
            preload: 预加载到GPU的样本数
        """
        self.features = []
        self.masks = []
        ...
        # 高效加载全部数据到RAM
        for i in tqdm(range(len(subset)), desc="加载数据集到内存"):
            feat, mask, target = subset[i]
            self.features.append(feat)
            self.masks.append(mask)
            self.targets.append(target)
            
        # 可选：将部分频繁使用的数据预加载到GPU
        if device is not None and preload > 0:
            n_preload = min(preload, len(self.features))
            self.features[:n_preload] = [f.to(device) for f in self.features[:n_preload]]
            self.masks[:n_preload] = [m.to(device) for m in self.masks[:n_preload]]
            self.targets[:n_preload] = [t.to(device) for t in self.targets[:n_preload]]
            self.has_gpu_data = True
            logger.info(f"预加载 {n_preload} 个样本到 GPU")
        else:
...
def main():
    ...
    # 为测试集创建内存数据集
    test_memory_ds = InMemoryDataset(test_set)
    test_loader = create_data_loader(test_memory_ds, args.batch_size, is_train=False)
    ...
    for fold, (train_subidx, val_subidx) in enumerate(kf.split(train_val_idx), 1):
        ...
        # 加载到内存
        train_memory_ds = InMemoryDataset(train_subset)
        val_memory_ds = InMemoryDataset(val_subset)

        # 创建数据加载器
        train_loader = create_data_loader(train_memory_ds, args.batch_size, is_train=True)
        val_loader = create_data_loader(val_memory_ds, args.batch_size, is_train=False)
        ...
        # 清理内存
        del train_memory_ds, val_memory_ds, train_loader, val_loader, model
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
